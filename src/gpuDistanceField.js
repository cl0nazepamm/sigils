...
export async function buildGpuDistanceField(renderer, paths, opts = {}) {
  ...
  // `storage` count is vec4 elements, not logical segments.
  ...
  const computeField = Fn(() => {
    ...
    Loop(activeSegments, ({ i }) => {
      ...
      // Segment data carries direction, inverse length and normalized arc
      // coefficients so this grid-cells × segments loop does no divisions.
      const vx = a.z;
      const vy = a.w;
      const wx = px.sub(a.x);
      const wy = py.sub(a.y);
  ...
  if (smoothPasses > 0) {
    try {
      ...
      // A lost device cannot recover on an immediate retry.
      if (renderer._isDeviceLost === true) throw smoothError;
      try {
        ...
        throw new AggregateError(
          [smoothError, retryError],
          'sigils: GPU field batch and raster-only retry both failed.',
        );
      }
    }
  }
...
function blurKernel(srcAttr, dstAttr, gw, gh, total, horizontal) {
  ...
  return Fn(() => {
    ...
    if (horizontal) {
      ...
      idxM = iy.mul(gridWidth).add(i0);
      idxP = iy.mul(gridWidth).add(i1);
    } else {
      const j0 = select(iy.greaterThan(0), iy.sub(1), iy);
      const j1 = select(iy.lessThan(gridHeight.sub(1)), iy.add(1), iy);
      idxM = j0.mul(gridWidth).add(ix);
      idxP = j1.mul(gridWidth).add(ix);
    }
...
class HybridDistanceField {
  constructor(paths, opts = {}) {
    ...
    const set = toPathSet(paths, { pointRadius: pointRadius === true });
