...
import { buildAdjacency } from './gpuLaplacian.js';
 ...
 * @param {number} [opts.taper] - field-style open-end blend 0..1 (preferred; matches DistanceField)
 * @param {number} [opts.taperLen=0.35] - legacy tip taper length when `taper` is omitted
 * @param {number} [opts.taperPower=0.6] - open-end taper exponent (field default when using `taper`)
 * @param {number} [opts.tipRadius=0] - minimum half-width at open tips
...
export function buildSparseCurveGeometry(paths, opts = {}) {
  const pathSet = toPathSet(paths, { pointRadius: opts.pointRadius === true });
  const positions = [];
  const uvs = [];
  const depths = [];
  const domes = [];
...
export async function buildSparseCurveGeometryAsync(renderer, paths, opts = {}) {
  if (opts.fieldMesh !== false && renderer) {
    const mergedOpts = {
      ...
      laplacian: opts.fieldLaplacian ?? opts.laplacian ?? 36,
    ...
    try {
      return await buildGpuFieldMeshAsync(renderer, paths, mergedOpts);
    } catch (error) {
      if (opts.onGpuFallback) opts.onGpuFallback(error);
      else console.warn('sigils: GPU field mesh failed; using CPU.', error);
      return buildSigilGeometry(paths, { ...mergedOpts, fieldBackend: 'cpu' });
    }
  }
...
function processStroke(stroke, opts) {
  ...
  // A position-only simplifier would erase a radius bulge along an otherwise
  // straight run. Weighted paths are already densely sampled by the spline.
  if (!rawClosed && !hasNonUnitRadius(points, opts)) {
    points = simplifyPolyline(points, opts.simplify ?? 0.006);
  }
...
function appendCurve(path, sectorIndex, mirrored, positions, uvs, depths, domes, normals, indices, opts) {
  ...
  // Sector transform is loop-invariant: compute cos/sin/center once and bake
  // every point of this path into flat arrays so p/pPrev/pNext are lookups.
  const center = opts.center ?? [0, 0];
  const angle = (opts.phase ?? 0) + (TAU / Math.max(1, Math.floor(opts.symmetry ?? 1))) * sectorIndex;
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (mirrored) x = -x;
    xs[i] = center[0] + x * cosA - y * sinA;
    ys[i] = center[1] + x * sinA + y * cosA;
  }

  for (let i = 0; i < count; i++) {
    const prevIndex = path.closed ? (i - 1 + count) % count : Math.max(0, i - 1);
    const nextIndex = path.closed ? (i + 1) % count : Math.min(count - 1, i + 1);
    let tx = xs[nextIndex] - xs[prevIndex];
    let ty = ys[nextIndex] - ys[prevIndex];
    ...
    rows.push({ px: xs[i], py: ys[i], nx, ny, half, along });
  ...
  for (let i = 0; i < count; i++) {
    ...
    for (let c = 0; c < topStride; c++) {
      ...
      positions.push(
        row.px + row.nx * row.half * cross,
        row.py + row.ny * row.half * cross,
        0,
      );
    ...
    if (hasBase) {
      const lx = row.px - row.nx * row.half;
      const ly = row.py - row.ny * row.half;
      const rx = row.px + row.nx * row.half;
      const ry = row.py + row.ny * row.half;
...
function pointHalfWidth(path, i, opts) {
  ...
  let half = baseHalf;
  if (!path.closed) {
    half = baseHalf * openEndTaperWeight(path, i, opts);
    const tipFloor = opts.tipRadius ?? (opts.taper == null ? 0.004 : 0);
    half = Math.max(tipFloor, half);
  }
  const scale = opts.pointRadius === true
    ? radiusScale(path.points[i]?.[2])
    : 1;
  return half * scale;
}

/** Match DistanceField._taperWeight when `opts.taper` is set; else legacy tip-length. */
function openEndTaperWeight(path, i, opts) {
  if (opts.taper != null && Number.isFinite(opts.taper)) {
    const taper = clamp01(opts.taper);
    if (taper <= 0) return 1;
    const s = path.total > 0 ? clamp01(path.distance[i] / path.total) : 0.5;
    const taperPower = Math.max(0.05, opts.taperPower ?? 0.6);
    const profile = Math.pow(Math.sin(Math.PI * s), taperPower);
    return 1 - taper * (1 - profile);
  }

  const taperLen = opts.taperLen;
  if (taperLen != null && !(taperLen > 0)) return 1;
  const terminalDistance = Math.min(path.distance[i], path.total - path.distance[i]);
  const len = Math.max(taperLen ?? 0.35, 1e-4);
  const t = clamp01(terminalDistance / len);
  return Math.pow(t, opts.taperPower ?? 1.8);
}

function hasNonUnitRadius(points, opts) {
  if (opts.pointRadius !== true) return false;
  return points.some((point) => Math.abs(radiusScale(point[2]) - 1) > 1e-12);
}
...
function computeDepthGradient(geometry) {
  ...
  const { neighborOffsets, neighborCount, neighborIndices } = buildAdjacency(count, index.array);
  const maxLen = 8;
