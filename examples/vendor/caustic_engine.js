// VENDORED from speedball/js/caustic_engine.js @ working (general receiver + t-cull WIP)
// Master lives in the speedball repo — do NOT edit here; update in speedball and re-vendor.
//
...
export function createCausticEngine({
    ...
    const emitTrace = (wp, wn, sourceGain) => {
        ...
        // Soft t-cull (the WIP named in the banner): long grazing throws smear
        // far from the caster and hijack the auto-exposure; fade them with the
        // virtual-source divergence so the caustic hugs the geometry.
    ...
    function setCasterMesh(mesh) {
        ...
        const bakeSigilHeight = depthAttr && gradAttr && domeAttr && peakHeight !== 0;
        ...
        for (let i = 0; i < vCount; i++) {
            ...
            if (bakeSigilHeight) {
                ...
                const heightProfile = profile === 'round'
                    ...
                    : depth;
                _v.z += peakHeight * heightProfile * dome;
            }
            _v.applyMatrix4(m);
            wpos[i * 3] = _v.x; wpos[i * 3 + 1] = _v.y; wpos[i * 3 + 2] = _v.z;
            if (bakeSigilHeight && domeAttr.getX(i) > 0.5) {
                const rawDepth = Math.max(0, depthAttr.getX(i));
                const depth = profile === 'round' ? Math.min(1, rawDepth) : rawDepth;
                ...
                _n.set(
                    ...
                    1,
                ).normalize().applyMatrix3(nm).normalize();
            } else if (nAttr) _n.fromBufferAttribute(nAttr, i).applyMatrix3(nm).normalize(); else _n.set(0, 1, 0);
