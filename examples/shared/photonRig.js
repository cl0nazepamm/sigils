/**
 * Photon blast rig — GPU caustics thrown off a committed sigil mesh onto a black
 * backdrop wall behind it. Shared by the Freehand and CV Curves modes: each
 * hands the rig its committed `sigilMesh` and drives the lifecycle hooks below.
 *
...
// across the front, lift raises it, range pulls it away. Glow drives both the
 ...
 * @param {HTMLElement} opts.button     the "Photon blast" toggle button
 ...
 */
export function createPhotonRig(ctx, { sigilMesh, state, button, panel, setStatus, signal }) {
  ...
  mountControlPanel(panel, PHOTON_CONTROL_SPECS, photonState, {
  ...
  function setActive(on) {
    if (on) {
      if (!hasCaster()) { setStatus?.('draw a sigil first'); return; }
      ensureRig();
      if (!uploadCaster()) return;
    }
    active = on;
    refreshVisuals();
    panel.hidden = !on;
    button.classList.toggle('active', on);
    button.textContent = on ? 'Photon off' : 'Photon blast';
  }
  ...
  return {
    ...
    syncCaster() { if (active) uploadCaster(); refreshVisuals(); },
    ...
    update() { if (active && caustic) caustic.update(); },
    dispose() {
